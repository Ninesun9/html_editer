...
- 通过 HtmlFox AI 面板调用 OpenAI-compatible 接口，按自然语言修改整页、预览选中元素或源码选区。
- 在 Monaco 中选中相关 HTML 后右键，可直接进入“AI 修改选中内容”的对话式编辑流程。
- AI 设置支持连接测试，便于确认 Base URL、模型和 API Key 是否可用。
...
- AI 修改：点击工具栏 `AI` 打开 HtmlFox AI，填写 Base URL、Model 和 API Key 后，可以测试连接并用自然语言修改整个文档或预览区选中的元素。
- 定点 AI 修改：在左侧 Monaco 中选中一段 HTML，右键选择 `HtmlFox AI：修改选中的 HTML`，随后通过 AI 对话持续修改这段源码范围。
...
- Use the HtmlFox AI panel with an OpenAI-compatible endpoint to edit the whole document, selected preview element, or selected source range using natural language.
- Select HTML in Monaco, right-click, and start a focused AI edit conversation for that exact range.
- Test AI connectivity from the settings area before running edits.
...
- AI editing: click `AI` in the toolbar, configure Base URL, Model, and API key, test the connection, then describe the change you want for the whole document or selected preview element.
- Focused AI editing: select HTML in Monaco, right-click `HtmlFox AI: Modify selected HTML`, then continue refining that exact source range through the AI conversation.
